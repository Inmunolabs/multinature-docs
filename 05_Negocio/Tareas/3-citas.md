...
- ✅ [03004] Completar el agendamiento de citas, de especialista a usuario y de usuario a especialista
  ...
  Context: Complemento de las tareas "[03002] Validar que no se pueda crear dos bookings iguales" y "[03003] Validar la perspectiva del usuario o especialista que crea la cita"
...
- ✅ [03005] Actualizar el bookings-api para implementar la nueva lógica de perfiles y especialidades
  ...
  Context: Trabajar desde el PR https://github.com/Inmunolabs/multinature-bookings-api/pull/3/files revisar como se implemento esta actualización en el api y corregirla para implementar correctamente la nueva lógica de perfiles y especialidades

- ✅ [03006] Agregar el id de la dirección y la url de videollamada y cambiar el reason por el id de la especialidad

  Context: Modificar la tabla de la base de datos, y el bookings-api, ojo en los endpoints `POST {{bookingsHost}}/bookings` y `PATCH /bookings/:id`
...
- ✅ [03007] Validar funcionalidad de actualización de cita
  ...
  Context: El especialista y el usuario involucrados en la cita deben ser capaces de modificarla, ya sea el horario, el lugar, la url de videollamada, o las notas.
...
- ✅ [02002] Agregar bandera para definir si la dirección es un consultorio de especialista
  ...
  Context: Modificar la base de datos y los endpoints
...
- ✅ [03010] Crear webhook del pago de la confirmación de la cita
  ...
  Context: Una vez que el usaurio efectue el pago de la confirmación de la cita se deberá notificar al usuario y al especialista la confirmación de la misma ([19007] Notificar al usuario y al especialista sobre la confirmación de una nueva cita). El objetivo de la tarear es crear el webhook que permitirá operar esta lógica.
...
- ✅ [03012] Confirmación de cita para especialistas
  ...
  Context: Cuando un usuario agende una cita, el especialista la confirma seleccionando el lugar de la cita. La lógica deberá quedar en el endpoint `PATCH /bookings/:id`. Tomar en cuenta validaciones como que los domicilios tengan la bandera `isClinic` en `true` y que las direcciones sean direcciones del especialista
...
- ✅ [03014] Actualizar el funcionamiento de agendamiento de las citas
  ...
  Context: Un usuario y un especialista deben ser capaces de agendarse citas respectivamente. **Revisar los detalles de esta tarea en la [documentación](https://github.com/Inmunolabs/multinature-docs/blob/master/4.%20Negocio/promptsDeTareas/%5B03%5D%20Citas.md)**
...
- ✅ [03015] Desarrollar opción y formularios para la actualización y cancelación de una cita
  ...
  Context: Un usuario y un especialista deben ser capaces de actualizar y cancelar citas respectivamente. **Revisar los detalles de esta tarea en la [documentación](https://github.com/Inmunolabs/multinature-docs/blob/master/4.%20Negocio/promptsDeTareas/%5B03%5D%20Citas.md)**.
...
- ✅ [03018] Validar que haya un domicilio al agendar/actualizar una citar o crear una url de videollamada automáticamente
  ...
  Context: Si al agendar o actualizar una cita no se tiene un domicilio agregado, crear una url de videollamada en Meet para atender la cita desde ahí. Revisar si esta funcionalidad solo será habilitada a los usuarios que tengan correo Gmail o si puede aplicar para todos.
...
- ✅ [03019] Integrar nuestras citas a Google Calendar
  ...
  Context: Esta funcionalidad solo sera habilitada a los usuarios que tengan correo Gmail. El objetivo es que puedan visualizar las citas de nuestro sistema desde sus Google Calendar
...
- ✅ [03020] Eliminar el estatus "Actualizada" de las citas
  ...
  Context: Eliminar el estatus "Actualizada" de las citas y actualizar todas las funcionalidades relacionadas con este estatus. Si hay alguna duda durante la tarea consultarla con Miguel
...
- ✅ [03024] Refactorización del bookings-api, actualización de endpoints
  ...
  Context: Refactorizar el api de bookings, unificar los siguientes endpoints en un solo PATCH que permita:
...
- ✅ [03027] No se muestran las formas de pago al pagar anticipo de citas con "Pago en tienda" o "Mercado Pago"
  ...
  Context: Al crear una cita no se muestra correctamente la información del pago, en el caso del Pago en Tienda, el PDF no es visible desde la url de la respuesta. En el caso de mercado pago, no se retorna la url para generar el pago.
...
- ✅ [03035] Los pacientes deben poder confirmar las citas creadas por el especialista como citas con pago completo en efectivo
  ...
  Context: Cuando un especialista crea una cita en efectivo con el pago completo, el paciente de la cita debe poder confirmarla sin necesidad de generar un pago, solo con un botón para confirmar.
