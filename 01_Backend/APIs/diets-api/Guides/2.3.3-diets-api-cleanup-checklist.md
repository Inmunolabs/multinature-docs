...
Este checklist documenta el estado de la simplificación arquitectónica del módulo **diets-api**, centralizando la lógica de generación en un pipeline unificado y reduciendo endpoints a solo los esenciales.
...
- [x] **Pipeline centralizado** en `src/services/pipeline.js`
  - [x] Función `computeEnergy()` implementada
  - [x] Función `resolveMacros()` implementada
  - [x] Función `buildSchedule()` implementada
  - [x] Función `applyEquivalences()` implementada
  - [x] Función `generateDishes()` implementada
...
- [x] **Guides/2.3.2-diet-pipeline-and-actions.md** creado
  - [x] Documentación detallada por tramo
  - [x] Ejemplos concretos de input/output
  - [x] Overrides disponibles
  - [x] Guías de debugging y testing

- [x] **Guides/2.3.3-diets-api-cleanup-checklist.md** creado (este archivo)

### 🗑️ Endpoints Legacy

- [x] Verificación de endpoints legacy
  - [x] `/macronutrients` - **NO EXISTE** en código actual
  - [x] `/portions` - **NO EXISTE** en código actual
  - [x] `/plates` - **NO EXISTE** en código current

**Conclusión:** No hay endpoints legacy que eliminar. Probablemente ya fueron removidos en refactors anteriores.
...
- [ ] Identificar y eliminar código sin referencias
...
- [ ] Instrumentar tiempos de ejecución
...
- [ ] Paralelización de queries en `generateDishes()`

### 📖 Documentación Adicional

- [ ] Agregar ejemplos de curl a README principal
- [ ] Documentar casos de error comunes
- [ ] Crear guía de migración para clientes existentes
...
Ejecutar las siguientes validaciones para confirmar que el refactor no rompió funcionalidad existente:
...
- [ ] Verificar estructura de `menus → meals → dishes`

#### 3. Validaciones de Negocio

- [ ] Plan siempre de 7 días (keys 0..6)
- [ ] Justificaciones en 1 párrafo (4-6 oraciones) con datos clínicos
...
### Issue #1: Validación de portionBalance puede fallar en casos extremos

**Descripción:** Para dietas muy bajas en calorías (<1200 kcal), el balance de porciones SMAE puede desviarse >10%.
...
### Issue #2: Tiempo de ejecución de generateDishes puede ser lento

**Descripción:** Para 7 días x 4 comidas = 28 meals, las queries a `foods` y `foods_ingredients` pueden ser lentas (~2-3s).

**Workaround:** Implementar cache en buildDishesFromPortions.

**Estado:** Optimización planeada
...
Si actualmente usas **POST /generate-automatic** con body explícito:

**Opción 1: Migrar a GET (recomendado)**
...
(sin body - todo se deriva automáticamente)
```

**Opción 2: Mantener POST (legacy)**

El endpoint POST sigue funcionando sin cambios. No requiere migración inmediata.
...
- Campo `mealPlan` → `menus`
- Campo `dailyDistribution` → `portionDistribution`
...
- [x] Documentación completa (README, Endpoints, Guides)
...
- ✅ Creación de pipeline unificado
- ✅ Implementación de POST /actions
- ✅ Documentación de endpoints y guías
...
- 🔴 Lógica de generación dispersa en múltiples servicios
- 🔴 Endpoint POST con body complejo obligatorio
- 🔴 No había forma de ejecutar tramos parciales
- 🔴 Documentación desactualizada

### Después

- ✅ Pipeline centralizado y modular
- ✅ Endpoint GET simple sin body
- ✅ Orquestador flexible para ajustes y pruebas
