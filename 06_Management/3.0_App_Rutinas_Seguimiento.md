# 3.0 App, Rutinas y Seguimiento – Documento Completo

## 🎯 Objetivo

Centralizar y estabilizar toda la lógica de **rutinas**, **PDFs**, **seguimiento del paciente** y **RoutinesAgent**, garantizando una experiencia sólida en app y panel.

---

# 1. EPICAS PRINCIPALES

## 1.1. PDFs de Rutinas (Profesional + Legible)

### Problemas actuales

- Formato básico / sin estilo.
- No incluye descansos, series, repeticiones correctamente.
- No refleja estructura semanal.
- No se integra con app.

### Entregables

- Generador PDF profesional.
- Layout semanal detallado.
- Logo, datos del paciente, datos del especialista.
- Exportación desde backend y panel.
- Versión para WhatsApp/email.

### Tareas

- [ ] Template PDF profesional.
- [ ] API para generar PDF por semana.
- [ ] Ajustar DTO para incluir descansos.
- [ ] Exportar desde panel (botón “Descargar PDF”).
- [ ] Integración app (descarga móvil).

---

## 1.2. CRUD de Rutinas por Semana

### Problemas

- Rutinas no están estructuradas por semana.
- UI confusa.
- Backend con campos dispersos.

### Entregables

- CRUD completo semanal.
- Asociación ejercicio ←→ día.
- Consistencia entre semana 1 y semana N.
- Eliminación de legacy.

### Tareas

- [ ] Endpoint CREATE semana.
- [ ] Endpoint UPDATE semana.
- [ ] Endpoint DELETE semana.
- [ ] Validaciones: días, ejercicios, repeticiones.
- [ ] Limpieza de código viejo.

---

## 1.3. RoutinesAgent (Generación + Edición)

### Problemas

- No existe lógica estable para IA.
- Sin modelo clínico de rutinas (fuerza, cardio, etc.)
- Falta editor manual.

### Entregables

- RoutinesAgent funcional.
- Generación automática por objetivo.
- Edición por parte del especialista.
- Integración completa con app.

### Tareas

- [ ] Modelo IA para rutinas.
- [ ] Endpoint generate routine.
- [ ] Editor visual (Diego).
- [ ] Trazabilidad de cambios.

---

## 1.4. Seguimiento del Paciente (App)

### Problemas

- App no tiene rutinas completas.
- Sin gráficas de progreso.
- Sin notificaciones.

### Entregables

- Rutinas visibles desde app.
- Marcar ejercicios completados.
- Progreso semanal.
- Notificaciones push.

### Tareas

- [ ] API seguimiento.
- [ ] UI seguimiento app (Pandalatec).
- [ ] Endpoint “complete exercise”.
- [ ] Gráficas progreso.

---

# 2. DEPENDENCIAS

1. PDFs dependen del CRUD semanal.
2. CRUD semanal depende del DTO limpio.
3. RoutinesAgent depende de CRUD + SMAE estable (tiempos, equivalentes).
4. App depende de API seguimiento funcional.

---

# 3. ASIGNACIÓN POR ROL

## Samuel

- CRUD backend.
- Generador PDF.
- RoutinesAgent backend.

## Miguel

- Arquitectura DTO rutinas.
- Integración con app.
- Reglas del agente.

## Diego

- UI CRUD.
- Editor IA.
- Descarga PDF.

## Erick

- Backend soporte CRUD.
- Validaciones.
- Tareas repetitivas.

## Cristopher

- Limpieza DTO.
- Testing manual.

## Antoine

- Diseño de rutinas.
- UX propias de entrenamiento.

## Leo

- QA completo de rutinas.
- QA de PDF.

---

# 4. STORY POINTS

| Épica           | SP  |
| --------------- | --- |
| PDFs            | 20  |
| CRUD Semanal    | 26  |
| RoutinesAgent   | 34  |
| Seguimiento App | 20  |

Total: **100 SP**

---

# 5. HORAS ESTIMADAS

| Rol        | Horas |
| ---------- | ----- |
| Samuel     | 70h   |
| Miguel     | 30h   |
| Diego      | 40h   |
| Erick      | 35h   |
| Cristopher | 18h   |
| Antoine    | 20h   |
| Leo        | 15h   |

Total: **228 horas**

---

# 6. ENTREGAS POR SPRINT

## Sprint 3 – Rumbo a 1.1

- CRUD semanal base
- PDFs versión 1
- API seguimiento

## Sprint 4 – Cierre 1.1

- RoutinesAgent completo
- Editor IA
- Progreso en app
- PDFs finales

---

# 7. RIESGOS

- Rutinas requieren más validación que dietas.
- App depende del partner externo.
- PDFs pueden ser lentos sin caché.

---

# 8. DOCUMENTACIÓN

- `/doc/routines-crud.md`
- `/doc/routines-agent.md`
- `/doc/pdf-routines.md`
