# 2.0 Operatividad del Especialista

## 🎯 Objetivo

Garantizar que los especialistas puedan **crear, editar, visualizar y administrar menús y platillos**, formularios y datos clínicos sin fricción.
Este bloque soporta **versión 1.0** y prepara el camino para **1.1**.

---

# 1. EPICAS PRINCIPALES

## _(Nice-to-have)_ 1.0. SMAE – Normalización y Cache

### Problemas actuales

- Etiquetas mixtas (mayúsculas, tildes, sinónimos).
- SMAE cargado _ad-hoc_ en cada cálculo → bajo performance.

### Entregables

- Normalización de etiquetas (acentos, mayúsculas, sinónimos).
- Carga del catálogo en startup.
- Cache local para evitar queries repetidas.

### Tareas

- [ ] Crear normalizador global de etiquetas.
  - [ ] Crear módulo `smaeNormalizer.js`.
  - [ ] Convertir strings → snake_case.
  - [ ] Remover acentos.
  - [ ] Unificar nombres similares (ej. “Aguacate Hass” vs “Aguacate”).
  - [ ] Mapa de sinónimos.
- [ ] Cargar SMAE completo en memoria al iniciar la API.
  - [ ] Crear servicio global `SMAE_CACHE`.
  - [ ] Cargar grupos, subgrupos y equivalentes al iniciar.
  - [ ] Exponer getters rápidos.
  - [ ] Evitar queries duplicadas en menus/diets.
- [ ] Documentar estructura final de SMAE.

---

## 1.1. CRUD de Menús y Alimentos (comidas)

### Problemas actuales

- Menús generados por IA o de manera manual no son editables desde la interfaz.
- Alimentos creados por especialistas no son editables desde la interfaz.
- No hay historial/versionado.
- DTO no está estandarizado.

### Entregables

- CRUD completo de menus y alimentos (crear, leer, actualizar, eliminar).
- Estructura única: menú manual o IA.
- Editor visual unificado.
- Historial y versionado.
- Validaciones de permisos (`userBelongsToSpecialist`).
- DTO estandarizado para frontend.

### Tareas

- [ ] CRUD menus.
- [ ] CRUD alimentos (foods).
- [ ] Unificar estructura IA vs manual.
- [ ] Integrar versionado.
- [ ] Documentar formato final: `/doc/menus-structure.md`.

---

## 1.2. Edición de Menús IA

### Problemas

- La IA produce JSON rígido.
- Especialistas no pueden ajustar porciones, horarios o platillos.
- Falta trazabilidad de cambios.

### Entregables

- Editor visual completo.
- Guardar cambios en JSON.
- Trazabilidad (quién lo cambió, cuándo).
- Ajustes dinámicos a partir del catálogo SMAE corregido.

### Tareas

- [ ] Endpoint PATCH para cambios granulares.
- [ ] UI de edición (Diego).
- [ ] Ajuste de horario/porciones/platillos.
- [ ] Guardar cambios con diff.
- [ ] Bitácora de cambios (`menus_history`).

---

## _(Nice-to-have)_ 1.3. Mejoras Formularios Clínicos de Pacientes

### Problemas

- Flujo de llenado lento.
- Inconsistencia en unidades.
- Conceptos duplicados.
- Validaciones débiles.
- UX confusa para especialistas.

### Entregables

- Mejorar agrupación lógica.
- Unificar unidades.
- Flujo lineal optimizado.
- Validaciones clínicas completas.
- Preparación para histórico graficable.

### Tareas

- [ ] Agrupación lógica de conceptos.
- [ ] Validaciones por tipo (peso, talla, glucosa, presión).
- [ ] Normalizar unidades.
- [ ] UI mejorada (Diego + Antoine).
- [ ] Documentación: `/doc/forms-structure.md`.

---

## _(Nice-to-have)_ 1.4. Historial Clínico Estructurado

### Problemas

- Navegación complicada.
- Historial disperso entre tablas.
- Pocas gráficas.
- Difícil para especialistas consultar evolución.

### Entregables

- Estructura temporal clara.
- Búsqueda por tipo de información.
- Filtros (signos vitales, bioquímicos, antropometría).
- Gráficas de evolución.
- Exportación PDF.

### Tareas

- [ ] Nueva vista frontend.
- [ ] API para filtrado.
- [ ] API para agrupación temporal.
- [ ] Gráficas (ReactCharts).
- [ ] Exportación PDF.
- [ ] Documentación: `/doc/clinical-history.md`.

---

## 1.5. Creación de Dieta a partir de la propuesta del Agente de Dietas

### Problemas

- El Agente de dietas responde con una propuesta de dieta, esta no es creada sino hasta que el especialista lo autoriza.

### Entregables

- Crear en base de datos la dieta completa del paciente.

### Tareas

- Crear en base de datos la dieta completa del paciente.

---

# 2. DEPENDENCIAS

1. CRUD de menús depende de:
   - Estabilidad Nutricional (SMAE + MealTimes + Porciones + GET)
2. Edición IA depende de:
   - GET estable
   - DTO consistente
3. Formularios dependen de:
   - Migraciones corregidas
   - Validaciones clínicas
4. Historial clínico depende de:
   - Formularios coherentes
   - DTO graficable

---

# 3. ASIGNACIÓN POR ROL

## Miguel

- Arquitectura CRUD menús.
- Definición DTO backend-frontend.
- Trazabilidad de cambios IA.

## Samuel

- Endpoints CRUD.
- Normalización DTO.
- Versionado.
- Integración edición IA.

## Diego

- UI completa: CRUD + Editor IA + formularios clínicos + historial.
- Gráficas de evolución.

## Antoine

- Diseño clínico.
- UX de formularios.
- Flujo de historial.
- **_NOTA:_** Antoine ya desarrollo la Epica [1.5. Creación de Dieta a partir de la propuesta del Agente de Dietas](#15-creación-de-dieta-a-partir-de-la-propuesta-del-agente-de-dietas)

## Erick

- Endpoints secundarios.
- Validaciones.
- Soporte a Samuel.

## Cristopher

- Limpieza DTO.
- Formatos de historial.

## Leo

- QA de formularios.
- QA de CRUD menús.
- QA de editor IA.

---

# 4. STORY POINTS

| Épica                | SP  |
| -------------------- | --- |
| CRUD Menús           | 34  |
| Editor IA            | 21  |
| Formularios Clínicos | 20  |
| Historial Clínico    | 26  |

Total: **101 SP**

---

# 5. HORAS TOTALES ESTIMADAS

| Rol        | Horas |
| ---------- | ----- |
| Miguel     | 35h   |
| Samuel     | 70h   |
| Diego      | 48h   |
| Antoine    | 20h   |
| Erick      | 28h   |
| Cristopher | 22h   |
| Leo        | 18h   |

Total estimado: **241h**

---

# 6. ENTREGA POR SPRINT

## Sprint 2 – Rumbo a 1.0

- CRUD Menús base
- DTO unificado
- Editor IA funcional en 60%
- Formularios reorganizados

## Sprint 3 – Rumbo a 1.1

- Historial clínico completo
- Editor IA final
- Validaciones finales
- Exportación PDF

---

# 7. RIESGOS

- DTO inestable → rompe frontend.
- Formularios aún dependientes de migraciones viejas.
- Historial clínico requiere refinamiento continuo.
- Editor IA puede crecer en complejidad.

---

# 8. DOCUMENTACIÓN FINAL

- `/doc/menus-structure.md`
- `/doc/menus-history.md`
- `/doc/forms-structure.md`
- `/doc/clinical-history.md`
