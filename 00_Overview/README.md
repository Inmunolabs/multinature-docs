# 00 Overview

Visión general del proyecto Multinature, incluyendo reglas de negocio, estructura del proyecto y guías para agentes de IA.

---

## 📑 Secciones

- **[Business Rules](./Business_Rules/README.md)** - Reglas de negocio y lógica del dominio

---

## 📄 Archivos Principales

- **[AGENTS.md](./AGENTS_GUIDE.md)** - Guía completa para agentes de IA (Codex, Cursor, etc) trabajando en el monorepo de Multinature
- **[Estructura del Proyecto](./PROJECT_STRUCTURE.md)** - Mapa visual y descripción de la organización del código backend

---

## 🎯 Contenido

### Para Desarrolladores Nuevos

1. **Empieza aquí:** [AGENTS.md](./AGENTS_GUIDE.md) - Guía completa del monorepo
2. **Estructura:** [PROJECT_STRUCTURE.md](./PROJECT_STRUCTURE.md) - Entiende la organización
3. **Reglas de Negocio:** [Business Rules](./Business_Rules/README.md) - Conoce la lógica del dominio

### Reglas de Negocio por Dominio

- [Usuarios](./Business_Rules/users/README.md)
- [Nutriólogos](./Business_Rules/nutriologos/README.md)
- [Especialistas](./Business_Rules/specialists/README.md)
- [Citas](./Business_Rules/citas/README.md)
- [Nosotros](./Business_Rules/nosotros/README.md)

---

- **Última actualización:** 2025-11-24
- **Total de archivos:** 2
